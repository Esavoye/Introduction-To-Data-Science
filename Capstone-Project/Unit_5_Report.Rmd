...
top_genre_scores
top_vec <- top_genre_scores$genre_group

df <- IGN_data_cleaned[IGN_data_cleaned$genre_group %in% top_vec,]
df                       
                       
colourCount = length(unique(top_genres$genre_group))
fill_purple <- colorRampPalette(brewer.pal(9,"BuPu"))

top_genres_score_plot <- df %>% ggplot(aes(x = genre_group, y = score, fill = genre_group, color = genre_group)) +
