...
top_platform_scores <- IGN_data_cleaned %>%
  group_by(platform_group) %>%
  summarize(sum_platforms_count = n()) %>%
...
Prior to any ventures into modeling, a data set was created to remove any unnecessary features as well as combine release day, month, and year into a combined date field. 

```{r subset_data, tidy=TRUE}

IGN_DC_PredM <- IGN_data_cleaned[,-c(1,3,4,5,7)]

IGN_DC_PredM <- IGN_DC_PredM %>% 
  unite(release_date, release_day, release_month, release_year, sep="/") 

IGN_DC_PredM$editors_choice.f <- factor(IGN_DC_PredM$editors_choice) 
IGN_DC_PredM$platform_group.f <- factor(IGN_DC_PredM$platform_group) 
IGN_DC_PredM$genre_group.f <- factor(IGN_DC_PredM$genre_group) 
...
ScoreReg <- lm(score ~ editors_choice.f + platform_group.f + genre_group.f, data = IGN_DC_PredM)
summary(ScoreReg)


```

Multicollinearity between score and editor's choice. low R-squared but not sure this is the proper method/regression for analyzing this data.
...
spl <- sample.split(IGN_DC_PredM$score, SplitRatio = 0.7)
Train <- subset(IGN_DC_PredM, spl == TRUE)
Test <- subset(IGN_DC_PredM, spl == FALSE)

# CART
IGN_Tree <- rpart(score ~ editors_choice.f + platform_group.f + genre_group.f, data = Train, method = "class", minbucket = 25)
prp(IGN_Tree)

# Random Forest
IGN_Forest <- randomForest(score ~ editors_choice.f + platform_group.f + genre_group.f, data = Train, nodesize = 25, ntree = 200)
