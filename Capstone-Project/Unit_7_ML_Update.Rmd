...
IGN_DC_PredM <- IGN_data_cleaned[,-c(2,3,5)]
...
ScoreReg <- lm(score ~., data = IGN_DC_PredM_WONA)
summary(ScoreReg)

#unique(IGN_DC_PredM$genre_group)
IGN_MM <- model.matrix(~ editors_choice + platform_group + genre_group, data=IGN_DC_PredM_WONA)
dim(IGN_MM)

IGN_MM <- cbind(IGN_MM,release_year = IGN_DC_PredM_WONA$release_year,release_month = IGN_DC_PredM_WONA$release_month,release_day = IGN_DC_PredM_WONA$release_day, score = IGN_DC_PredM_WONA$score)

sapply(IGN_DC_PredM_WONA, function(x) sum(is.na(x)))
IGN_DC_PredM_WONA <- na.omit(IGN_DC_PredM)
dim(IGN_DC_PredM_WONA)
