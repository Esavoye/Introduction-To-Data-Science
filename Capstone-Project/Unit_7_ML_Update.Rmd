...
***
# Predictive Models - Editor's Choice or Not
...
IGN_DC_PredM <- IGN_DC_PredM_WONA[,-c(2,3,5)]

str(IGN_DC_PredM)

IGN_MM <- model.matrix(~ editors_choice + platform_group + genre_group, data=IGN_DC_PredM)
dim(IGN_MM)

IGN_MM <- cbind(IGN_MM,release_year = IGN_DC_PredM$release_year,release_month = IGN_DC_PredM$release_month,release_day = IGN_DC_PredM$release_day, score = IGN_DC_PredM$score)
...
ScoreReg <- lm(score ~ . - score_phrase, data = IGN_DC_PredM)
summary(ScoreReg)
...
Previous iterations of the linear regression showed multicollinearity between score and score phrase with a multiple R-squared of 0.9726. After removing score phrase and re-running, R-squared drops to 0.3187.

#### Logistic Regression

```{r GLM, tidy=TRUE}

table(IGN_DC_PredM$editors_choice)
...
editchoice <- glm(editors_choice ~ ., data = Train, family = "binomial")
...
# CART
IGN_Tree <- rpart(score ~ editors_choice + platform_group + genre_group, data = Train, method = "class", minbucket = 3)
prp(IGN_Tree)
prp(IGN_Tree)

# Random Forest
IGN_Forest <- randomForest(score ~ editors_choice + platform_group + genre_group, data = Train, nodesize = 2, ntree = 200)
